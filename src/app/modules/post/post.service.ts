...
const createPostIntoDB = async(userId:string,payload:IPost)=>{
 ...
 // Checking is the post created successfully
    ...
    const updatedUserStatus = await User.updateOne({_id:objectId(userId)},{$inc:{total_post:1}})
    ...
    await session.endSession()
    throw new AppError(httpStatus.BAD_REQUEST,'Post can not be created.something went wrong')
   }
}

const getPostsFromDB = async (query:any)=>{
    let result = await new QueryBuilder(Post.find(),query).find().textSearch().sort().paginate().populate('author').get()
...
export const updatePostIntoDB = async (userId:string,postId:string,payload:Partial<IPost>)=>{
...
//  Checking post existence 
 ...
 try {
  ...
  const userUpdateStatus = await User.updateOne({_id:objectId(userId)},{$inc:{total_post:-1}},{session})
  ...
  await session.abortTransaction()
  await session.endSession()
  throw new AppError(httpStatus.BAD_REQUEST,'Post can not be deleted something went wrong')
 }
}
