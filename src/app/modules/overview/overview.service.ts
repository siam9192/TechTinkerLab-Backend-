...
import { getDaysInMonth, getFirstDayOfMonth, getLastDayOfMonth, objectId } from "../../utils/function"
...
const getPostOverviewDataFromDB = async (postId:string,query:{view_type:'date'|'month'|'year'})=>{
...
if(query.view_type === 'month'){
  const comments = await Comment.find({createdAt: {$lte:last_date_of_Year} })
  
 comments_summery = months.map((month,monthNumber)=>{
  const total_comment = comments.filter((comment) => new Date(comment.createdAt).getMonth() === monthNumber ).length
  return {
  month:month,
  total_comment,
  upcoming:new Date().getMonth()<monthNumber // Checking  upcoming month (if month number is getter than  current month number then true otherwise false)
 ...
 const reactions = await Reaction.find({createdAt: {$lte:last_date_of_Year}})
 reactions_summery = months.map((month,monthNumber)=>{
  const total_reaction = reactions.filter((reaction)=>  new Date(reaction.createdAt).getMonth() === monthNumber ).length
  return {
    month:month,
    total_reaction,
    upcoming:new Date().getMonth()<monthNumber // Checking  upcoming month (if month number is getter than  current month number then true otherwise false)
 ...
 const readers = await Reader.find({createdAt: {$lte:last_date_of_Year}})
   readers_summery = months.map((month,monthNumber)=>{
    const total_reader = readers.filter((reader)=>  new Date(reader.createdAt).getMonth() === monthNumber ).length
  ...
  const firstDayOfMonth = getFirstDayOfMonth(current_year,current_month_number)
  const lastDayOfMonth = getLastDayOfMonth(current_year,current_month_number)
  
  const commentsOfCurrentDate = await Comment.find({createdAt: {$gte:firstDayOfMonth,$lte:lastDayOfMonth} })

  const readersOfCurrentDate = await Reader.find({createdAt: {$gte:firstDayOfMonth,$lte:lastDayOfMonth} })

  const reactionsOfCurrentDate = await Reaction.find({createdAt: {$gte:firstDayOfMonth,$lte:lastDayOfMonth} })
  
  comments_summery = monthDates.map(date=>{
    const total_comment = commentsOfCurrentDate.filter((comment)=>new Date(comment.createdAt).getDate() === date).length
    return {
      date,
      total_comment,
 ...
 readers_summery = monthDates.map(date=>{
    const total_reader = reactionsOfCurrentDate.filter((reader)=>new Date(reader.createdAt).getDate() === date).length
    return {
      date,
      total_reader,
      upcoming:new Date().getMonth()< date// Checking  upcoming day (if month number is getter than  current date number then true otherwise false)
  ...
  reactions_summery = monthDates.map(date=>{
     const total_reaction = commentsOfCurrentDate.filter((comment)=>new Date(comment.createdAt).getDate() === date).length
     return {
      date,
      total_reaction,
      upcoming:new Date().getMonth()< date// Checking  upcoming day (if month number is getter than  current date number then true otherwise false)
